...
class Prompt2PromptXLPipeline(StableDiffusionXLPipeline):
    r"""
    Args:
    ...
    def __call__(
        ...
        self.controller = create_controller(
            prompt, cross_attention_kwargs, num_inference_steps, tokenizer=self.tokenizer, device=self.device, dtype=self.text_encoder.dtype
        )
...
def create_controller(
    prompts: List[str], cross_attention_kwargs: Dict, num_inference_steps: int, tokenizer, device, dtype
) -> AttentionControl:
    ...
    if edit_type == "replace" and local_blend_words is None:
        return AttentionReplace(
            prompts, num_inference_steps, n_cross_replace, n_self_replace, tokenizer=tokenizer, device=device, dtype=dtype
        )

    # replace + localblend
    if edit_type == "replace" and local_blend_words is not None:
        lb = LocalBlend(prompts, local_blend_words, tokenizer=tokenizer, device=device,dtype=dtype)
        return AttentionReplace(
            prompts, num_inference_steps, n_cross_replace, n_self_replace, lb, tokenizer=tokenizer, device=device,dtype=dtype
        )
...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str):
        ...
        if attn.shape[1] <= 48**2:  # avoid memory overhead
...
class LocalBlend:
    def __call__(self, x_t, attention_store):
        ...
        maps = [item.reshape(self.alpha_layers.shape[0], -1, 1, 24, 24, self.max_num_words) for item in maps]
    ...
    def __init__(
        self, prompts: List[str], words: [List[List[str]]], tokenizer, device, threshold=0.3, dtype=None
    ):
        ...
        self.alpha_layers = alpha_layers.to(device=device,dtype=dtype)
...
class AttentionReplace(AttentionControlEdit):
    ...
    def __init__(
        ...
        self.mapper = get_replacement_mapper(prompts, self.tokenizer).to(device=self.device,dtype=dtype)
...
class AttentionRefine(AttentionControlEdit):
    ...
    def __init__(
        ...
        self.mapper, alphas = self.mapper.to(self.device), alphas.to(device=self.device,dtype=dtype)
